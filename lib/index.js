...
class Animator {
  ...
  get playState(){
    ...
    if(timeline.duration < 0){ // 开始 pending
      return 'pending'
    }
    
    const ed = timeline.duration - iterations * duration
  ...
  get progress() {
    ...
    if(this.playState === 'paused' && timeline.duration < 0){
      return 0
    }
    ...
    if(this.playState === 'pending') {
      if(timeline.duration < 0){
        ...
        const time = timeline.seekTime(iterations * duration)
  ...
  play() {
    if(this.playState === 'idle' || this.playState === 'finished'){
      ...
      if(this[_finishedDefer] && !this[_finishedDefer].timerID) {
        ...
        }, duration * iterations + endDelay - this.timeline.duration)
      }
  ...
  finish() {
    this.timeline.duration = Infinity
    if(this[_readyDefer]){
      this.timeline.clearTimeout(this[_readyDefer].timerID)
      delete this[_readyDefer]
    }
